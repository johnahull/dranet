...
func defaultGatewayInterfaces() map[string]bool {
	...
	for _, path := range []string{"/proc/1/net/ipv6_route", "/proc/net/ipv6_route"} {
		...
		for _, line := range strings.Split(string(data), "\n") {
			...
			// Destination 00000000000000000000000000000000 prefix 00 = ::/0
...
// FilterVFIOUnsafe removes devices marked as vfio-unsafe from the list.
// Use this instead of MarkVFIOUnsafe when you want to completely hide
// unsafe devices from the ResourceSlice rather than annotating them.
func FilterVFIOUnsafe(devices []resourcev1.Device) []resourcev1.Device {
	var safe []resourcev1.Device
	for _, dev := range devices {
		pciAddr := getPCIAddr(&dev)
		if pciAddr == "" {
			safe = append(safe, dev)
			continue
		}
		ifName := getIfName(&dev)

		if hasSharedIommuGroup(pciAddr) {
			klog.Infof("Excluding %s (pci=%s): shared IOMMU group", dev.Name, pciAddr)
			continue
		}
		if ifName != "" {
			gw := defaultGatewayInterfaces()
			if gw[ifName] {
				klog.Infof("Excluding %s (if=%s): default gateway interface", dev.Name, ifName)
				continue
			}
		}
		safe = append(safe, dev)
...
func init() {
	// Verify we can read IOMMU groups (may not work without /sys mount)
	if _, err := os.ReadDir("/sys/kernel/iommu_groups"); err != nil {
		fmt.Fprintf(os.Stderr, "warning: cannot read IOMMU groups: %v\n", err)
	}
}
